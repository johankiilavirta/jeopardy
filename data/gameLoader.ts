...
// data/seasonFiles.ts is generated by scripts/generate-season-registry.ts:
// a switch of static require() calls (Metro can't bundle dynamic paths)
// that parse lazily, one season at a time. Parsed seasons are cached here.

const _seasonCache = new Map<string, GameData[] | null>();

function getSeason(file: string): GameData[] | null {
  if (!_seasonCache.has(file)) _seasonCache.set(file, loadSeasonFile(file));
  return _seasonCache.get(file) ?? null;
}
...
export function loadGame(gameNumber: number): GameData | null {
  ...
  const games = getSeason(season.file);
...
/**
 * Read selection metadata from the bundled archive. Nearby and Bluetooth
 * games use this path so the lobby never needs an internet relay merely to
 * preview a chosen game number.
 */
export function loadGameInfo(gameNumber: number): GameInfo | null {
  const game = loadGame(gameNumber);
  if (!game) return null;

  const seasonFile = loadGameIndex().seasons.find(
    season => gameNumber >= season.startGame && gameNumber <= season.endGame,
  )?.file;
  const year = seasonFile ? Number.parseInt(seasonFile.replace(/^season-|\.json$/g, ''), 10) : NaN;
  ...
  return {
    ...
    season: Number.isFinite(year) ? year - 1983 : null,
